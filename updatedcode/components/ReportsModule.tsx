...
const ReportsModule: React.FC<ReportsModuleProps> = ({ leads, bookings, inventory }) => {
    ...
    return (
        <div className="h-full flex flex-col bg-slate-50 animate-fade-in">
            {/* Header */}
            <div className="bg-white border-b border-slate-200 px-4 md:px-6 py-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                ...
                <div className="flex gap-3 w-full md:w-auto">
                    <button onClick={() => alert("Report Scheduled for Weekly Email!")} className="flex-1 md:flex-none px-4 py-2 bg-slate-100 text-slate-700 rounded-lg text-sm font-bold hover:bg-slate-200">
                        Schedule
                    </button>
                    <button onClick={() => alert("Report Saved to 'My Folder'")} className="flex-1 md:flex-none px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700 flex items-center justify-center gap-2">
            ...
            <div className="flex bg-white border-b border-slate-200 overflow-x-auto no-scrollbar">
                {['Select Source', 'Filter Logic', 'Grouping & Metrics', 'Visualize'].map((label, i) => (
                    <div 
                        key={i} 
                        onClick={() => setStep(i + 1)}
                        className={`flex-1 min-w-[140px] py-3 text-center text-xs font-bold uppercase border-b-4 cursor-pointer transition whitespace-nowrap px-2 ${
            ...
            <div className="flex-1 p-4 md:p-8 overflow-y-auto">
                ...
                {step === 1 && (
                    <div className="max-w-4xl mx-auto">
                        <h2 className="text-xl md:text-2xl font-bold text-slate-800 mb-6 text-center">What data do you want to analyze?</h2>
                    ...
                    <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-sm border border-slate-200 p-4 md:p-8">
                        ...
                        <div className="space-y-4 mb-6">
                            {config.filters.map((f, idx) => (
                                <div key={f.id} className="flex flex-col md:flex-row gap-3 items-start md:items-center bg-slate-50 p-3 rounded-lg border border-slate-200">
                                    <span className="text-xs font-bold text-slate-400 md:w-6 text-center hidden md:block">{idx + 1}.</span>
                                    
                                    <div className="flex gap-3 w-full md:w-auto">
                                        <span className="text-xs font-bold text-slate-400 md:hidden pt-2">{idx + 1}.</span>
                                        <select 
                                            value={f.field}
                                            onChange={(e) => updateFilter(f.id, 'field', e.target.value)}
                                            className="flex-1 p-2 border border-slate-300 rounded text-sm w-full"
                                        >
                                    ...
                                    <select 
                                        ...
                                        className="w-full md:w-32 p-2 border border-slate-300 rounded text-sm"
                                    >
                                    ...
                                    <input 
                                        ...
                                        placeholder="Value..."
                                    />
                                    <button onClick={() => removeFilter(f.id)} className="self-end md:self-center p-2 text-red-500 hover:bg-red-50 rounded">
                ...
                {step === 4 && (
                    <div className="space-y-6">
                        ...
                        {/* Chart Canvas */}
                        <div className="bg-white p-4 md:p-8 rounded-xl border border-slate-200 shadow-lg min-h-[400px] flex flex-col">
                                    ...
                                    <div className="h-[400px] w-full">
                                        <ResponsiveContainer width="100%" height="100%">
                                            {config.chartType === 'Bar' ? (
                                                ...
                                                <PieChart>
                                                    <Pie data={resultData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={120} label>
