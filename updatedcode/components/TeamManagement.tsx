...
import { Users, UserPlus, GitMerge, Edit2, Save, ToggleLeft, Clock, Phone, TrendingUp, Eye, BarChart2, ArrowRightLeft, X, MapPin, CheckCircle } from 'lucide-react';
...
const TeamManagement: React.FC<TeamManagementProps> = ({ agents, setAgents, leads, onUpdateLead }) => {
  const [activeTab, setActiveTab] = useState<'list' | 'chart'>('chart'); // Default to Chart
  ...
  const [orgData, setOrgData] = useState<OrgNode>({
      id: '1', name: 'Ashish Sonawane', role: 'Director', 
      children: [
          {
              id: '2', name: 'Deepesh', role: 'Sales VP',
              children: [
                  {
                      id: '3', name: 'Sales Head', role: 'Sales Head',
                      children: [
                          { id: '4', name: 'Closing Manager', role: 'Sales Manager' },
                          { 
                              id: '5', name: 'Diya Nalawar', role: 'Presales Team Leader',
                              children: [
                                  { id: '6', name: 'Presales Team', role: 'Executives' }
                              ]
                          }
                      ]
                  }
          ...
          {
              id: '7', name: 'Niranjan Sathe', role: 'Marketing Manager',
              children: [
                  { id: '8', name: 'Yogesh Chaurasia', role: 'Assistant Manager Marketing' }
              ]
          }
      ]
  });
  ...
  const renderTree = (node: OrgNode) => {
      return (
          <div key={node.id} className="flex flex-col items-center">
              <div className={`relative p-4 rounded-xl border-2 shadow-sm bg-white min-w-[180px] text-center transition hover:shadow-md hover:-translate-y-1 z-10
                  ${node.role.includes('Director') ? 'border-purple-200 bg-purple-50' : 
                    node.role.includes('VP') ? 'border-blue-200 bg-blue-50' : 
                    'border-slate-200'}`}
              >
                  <div className="font-bold text-slate-800 text-sm">{node.name}</div>
                  <div className="text-xs text-slate-500 uppercase font-medium mt-1">{node.role}</div>
                  
                  {/* Edit Button (Simulation) */}
                  <button className="absolute -top-2 -right-2 bg-white border border-slate-200 p-1 rounded-full text-slate-400 hover:text-blue-600 shadow-sm">
              ...
              {node.children && node.children.length > 0 && (
                  <>
                      <div className="w-px h-6 bg-slate-300"></div> {/* Vertical Line Down */}
                      <div className="flex gap-8 relative">
                          {/* Horizontal Connector Bar */}
                          <div className="absolute top-0 left-0 right-0 h-px bg-slate-300 -z-0" style={{ 
                          ...
                          {node.children.map((child) => (
                              <div key={child.id} className="flex flex-col items-center pt-6 relative">
                                  <div className="absolute top-0 w-px h-6 bg-slate-300"></div> {/* Vertical Line to Child */}
                                  {renderTree(child)}
                              </div>
                          ))}
                      </div>
    ...
    <div className="h-full flex flex-col bg-slate-50 p-6 space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        <div>
            ...
            <p className="text-slate-500 text-sm">Manage reporting structures and daily performance.</p>
        </div>
        <div className="flex bg-white rounded-lg border border-slate-200 p-1 shadow-sm">
            <button onClick={() => setActiveTab('list')} className={`px-4 py-2 rounded-md text-sm font-medium transition ${activeTab === 'list' ? 'bg-slate-800 text-white shadow' : 'text-slate-600 hover:bg-slate-50'}`}>Roster & Stats</button>
            <button onClick={() => setActiveTab('chart')} className={`px-4 py-2 rounded-md text-sm font-medium transition ${activeTab === 'chart' ? 'bg-slate-800 text-white shadow' : 'text-slate-600 hover:bg-slate-50'}`}>Org Chart</button>
        </div>
      </div>

      {activeTab === 'chart' ? (
          <div className="bg-slate-100 border border-slate-200 rounded-xl shadow-inner p-10 overflow-auto flex justify-center min-h-[600px]">
              <div className="pt-10">
                  {renderTree(orgData)}
              </div>
          </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                  <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                      <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                          <Users className="w-5 h-5 text-blue-600" /> Agent Attendance & Roster (Today)
                      </h3>
                  </div>
