...
import { 
    ...
    TrendingUp, DollarSign, Send, ShieldCheck, Eye, Check, X, Trophy, Medal, Award, Lock, ChevronRight
} from 'lucide-react';
...
const ChannelPartnerModule: React.FC<ChannelPartnerModuleProps> = ({ 
    ...
    // Admin State
    const [activeTab, setActiveTab] = useState<'list' | 'approvals' | 'leaderboard'>('list');
    ...
    const handleGenerateQR = (partner: ChannelPartner) => {
        alert(`QR Generated for ${partner.firmName}!\n\nURL: estateflow.com/p/${partner.id}\n\nAny lead scanning this will be auto-tagged to this partner.`);
    };

    // --- PARTNER APP FUNCTIONS ---

    const checkLeadConflict = () => {
        if(!leadMobile || leadMobile.length < 10) return;
        setLeadCheckStatus('checking');
        
        // Simulate Network Delay for "Blind Check"
        setTimeout(() => {
            const conflict = leads.find(l => l.mobile.includes(leadMobile));
            // Rules: 
            // 1. If lead exists AND active < 45 days ago -> Conflict
            // 2. If lead exists but old -> Allowed (Re-registration)
            // 3. If unique -> Available
            
            if (conflict) {
                const lastActive = conflict.remarksHistory[conflict.remarksHistory.length - 1]?.timestamp;
                const daysAgo = lastActive ? Math.floor((Date.now() - new Date(lastActive).getTime()) / (1000 * 60 * 60 * 24)) : 0;
                
                if (daysAgo < 45) {
                    setLeadCheckStatus('conflict');
                    // BLIND CHECK: Do not show name. Only show status code.
                    setConflictDetails(`Error 409: Lead Locked. Owned by ${conflict.source === 'Channel Partner' ? 'another CP' : 'Internal Sales'}. Active ${daysAgo} days ago.`);
                } else {
                    setLeadCheckStatus('available'); // Re-register allowed
                }
            } else {
    ...
    const handleShareBrochure = (item: MarketingCollateral) => {
        // Simulation of White-labeling
        const confirmShare = confirm(`White-Label Engine:\n\n1. Removing Builder Contact...\n2. Inserting ${simulatedUser.firmName} Logo...\n3. Adding RERA: ${simulatedUser.reraId}\n\nProceed to Share?`);
        if (confirmShare) {
            alert("Generated! Opening WhatsApp with your branded PDF.");
        }
    };
    ...
    const baseRate = 2; // 2%
    const acceleratorThreshold = 5; // 5 Units
    const acceleratorRate = 2.5; // 2.5%
    const currentSales = leads.filter(l => l.subSource === simulatedUser.firmName && l.stage === LeadStage.BOOKED).length;
    const nextSlab = currentSales >= acceleratorThreshold ? 'Max Tier' : `${acceleratorThreshold - currentSales} units to ${acceleratorRate}%`;
            ...
            {viewMode === 'admin' && (
                <div className="flex-1 overflow-y-auto p-6">
                    ...
                    <div className="flex border-b border-slate-200 mb-6">
                        ...
                        <button onClick={() => setActiveTab('leaderboard')} className={`px-6 py-3 text-sm font-bold border-b-2 ${activeTab === 'leaderboard' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500'}`}>Leaderboard</button>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {partners.map(cp => (
                                        <tr key={cp.id} className="hover:bg-slate-50">
                                            ...
                                            <td className="px-6 py-4">
                                                <button onClick={() => handleGenerateQR(cp)} className="text-xs flex items-center gap-1 text-blue-600 hover:underline bg-blue-50 px-2 py-1 rounded border border-blue-100">
                        ...
                        <div className="max-w-4xl mx-auto space-y-6">
                            <div className="bg-purple-900 text-white p-8 rounded-xl shadow-2xl relative overflow-hidden">
                                <div className="relative z-10">
                                    <h2 className="text-3xl font-bold mb-2">Partner Championship 2024</h2>
                                    <p className="opacity-80">Top performers get an all-expenses-paid trip to Dubai! ✈️</p>
                                </div>
                                <Trophy className="absolute right-8 top-1/2 transform -translate-y-1/2 w-32 h-32 text-yellow-400 opacity-30" />
                            </div>

                            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                                {partners.sort((a,b) => b.totalSalesValue - a.totalSalesValue).map((cp, idx) => (
                                    <div key={cp.id} className="flex items-center p-4 border-b border-slate-100 last:border-0 hover:bg-slate-50">
                                        <div className="w-12 text-center font-bold text-xl text-slate-400">
                                            {idx === 0 ? <span className="text-3xl">🥇</span> : idx === 1 ? <span className="text-2xl">🥈</span> : idx === 2 ? <span className="text-2xl">🥉</span> : `#${idx+1}`}
                                        </div>
                                        <div className="w-12 h-12 bg-slate-200 rounded-full flex items-center justify-center text-slate-600 font-bold mr-4">
                                            {cp.firmName.charAt(0)}
                                        </div>
                                        <div className="flex-1">
                                            <h4 className="font-bold text-slate-800">{cp.firmName}</h4>
                                            <p className="text-xs text-slate-500">{cp.leadsCount} Leads Generated</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="font-bold text-emerald-600 text-lg">₹{(cp.totalSalesValue/10000000).toFixed(2)} Cr</p>
                                            <p className="text-xs text-slate-400">Sales Volume</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto bg-slate-50 custom-scrollbar pb-20">
                            ...
                            {appView === 'dashboard' && (
                                <div className="p-4 space-y-4">
                                    ...
                                    {/* Commission Calculator / Slabs */}
                                    <div className="bg-gradient-to-r from-indigo-500 to-purple-500 rounded-xl p-4 text-white relative overflow-hidden">
                                        <div className="flex justify-between items-center relative z-10">
                                            <div>
                                                <p className="text-xs opacity-80 uppercase font-bold">Current Slab</p>
                                                <p className="text-2xl font-bold">{currentSales >= acceleratorThreshold ? acceleratorRate : baseRate}%</p>
                                            </div>
                                            <TrendingUp className="w-8 h-8 opacity-50" />
                                        </div>
                                        <div className="mt-3 relative z-10">
                                            <div className="flex justify-between text-[10px] mb-1 opacity-90">
                                                <span>{currentSales} Sales</span>
                                                <span>Target: {acceleratorThreshold}</span>
                                            </div>
                                            <div className="w-full bg-black/20 h-1.5 rounded-full overflow-hidden">
                                                <div className="bg-white h-full transition-all duration-500" style={{ width: `${Math.min((currentSales/acceleratorThreshold)*100, 100)}%` }}></div>
                                            </div>
                                            <p className="text-[10px] mt-1 opacity-75">{nextSlab}</p>
                                        </div>
                                    </div>
                            ...
                            {/* APP VIEW: REGISTER LEAD (BLIND CHECK) */}
                            {appView === 'register' && (
                                <div className="p-6 space-y-6 bg-white min-h-full">
                                    <div>
                                        <h3 className="text-xl font-bold text-slate-800 mb-2">Conflict-Free Registration</h3>
                                        <p className="text-xs text-slate-500">Verify lead availability without exposing data.</p>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                            ...
                                            <div className="flex gap-2">
                                                <div className="relative flex-1">
                                                    ...
                                                    <input 
                                                        ...
                                                        className="w-full border border-slate-300 rounded-lg pl-10 pr-3 py-3 text-lg outline-none focus:border-blue-500 font-mono tracking-wider"
                                                        placeholder="98765XXXXX"
                                                    />
                                                </div>
                                                <button onClick={checkLeadConflict} className="bg-slate-100 px-4 rounded-lg font-bold text-slate-600 text-sm hover:bg-slate-200">Check</button>
                                            </div>
                                        </div>

                                        {leadCheckStatus === 'checking' && (
                                            <div className="text-blue-600 text-sm flex items-center gap-2 animate-pulse">
                                        ...
                                        {leadCheckStatus === 'conflict' && (
                                            <div className="bg-red-50 p-4 rounded-xl border border-red-100 flex items-start gap-3 animate-shake">
                                                ...
                                                <div>
                                                    <p className="font-bold text-red-700 text-sm">Lead Locked</p>
                            ...
                            {/* APP VIEW: INVENTORY & MARKETING (WHITE-LABEL) */}
                            {appView === 'inventory' && (
                                    ...
                                    <div className="space-y-3">
                                        {MOCK_COLLATERAL.map(item => (
                                            <div key={item.id} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex justify-between items-center">
                                                ...
                                                <button onClick={() => handleShareBrochure(item)} className="p-2 text-green-600 bg-green-50 rounded-lg hover:bg-green-100 flex items-center gap-1 text-xs font-bold">
