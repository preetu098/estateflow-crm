...
import SiteVisitModule from './components/SiteVisitModule';
...
import { Lead, Project, Agent, Booking, Unit, SiteVisit, PricingConfig, LeadSource, ChannelPartner, CommissionInvoice, MarketingCampaign, SecurityConfig, EmailTemplate, IntegrationConfig, FeatureFlags, ConstructionUpdate, Vendor, Tenant } from './types';
import { Plus, Menu, UserCircle, ChevronDown, HelpCircle, RefreshCw, ShieldAlert, Bell, Check, X, Search } from 'lucide-react';
...
interface Notification {
  ...
  timestamp: Date;
  read: boolean;
  audience: ('Presales' | 'Sales' | 'TeamLeader' | 'SuperAdmin')[];
}
...
const App: React.FC = () => {
  ...
  const triggerNotification = (title: string, message: string, audience: Notification['audience']) => {
    ...
    // Here we update local state and play a sound.
    setNotifications(prev => [newNotif, ...prev]);
    
    // Simulate "Toast" sound (browser policy might block autoplay)
    try {
  ...
  const handleSaveLead = (lead: Lead) => {
    setLeads(prev => {
      ...
      if (exists) {
        ...
        // Trigger Notification for TL and Sales Exec
        triggerNotification(
          "New Lead Assigned",
          `Lead ${finalLead.name} has been assigned to ${finalLead.agentName || 'Team'}.`,
          ['Presales', 'TeamLeader']
        );
  ...
  const handleAddVisit = (visit: SiteVisit) => {
      ...
      // Trigger Notification for Sales Manager & TL
      triggerNotification(
        "Site Visit Scheduled",
        `Client ${visit.visitorName} is visiting ${visit.project} at ${new Date(visit.checkInTime).toLocaleTimeString()}.`,
        ['Sales', 'TeamLeader']
      );
  ...
  const handleCommandAction = (action: string, payload?: any) => {
      if (action === 'add_lead') {
          setView('leads');
          openAddModal();
      } else if (action === 'edit_lead' && payload) {
          setView('leads');
          openEditModal(payload as Lead);
      }
  };
  ...
  // Helper to check if a module is allowed for current view
  // This maps view names to feature flag keys and handles "Core" modules
  const isModuleAllowed = (viewName: string): boolean => {
      // Core modules that are always allowed
      const coreModules = ['dashboard', 'leads', 'settings', 'team', 'sales', 'developer-hub', 'archival', 'super-admin', 'quality-audit', 'landing-pages', 'cockpit', 'incentives'];
      if (coreModules.includes(viewName)) return true;

      // Feature Flag Mapping
      const map: Record<string, keyof FeatureFlags> = {
          'marketing': 'marketingModule',
          'nurture': 'presalesDialer',
          'reception': 'visitEngine',
          'channel-partners': 'cpModule',
          'bookings': 'postSales',
          'visits': 'visitEngine',
          'construction': 'constructionModule',
          'operations': 'operationsModule',
          'customer-portal': 'postSales', // Part of post-sales usually, or separate
          'procurement': 'procurementModule',
          'legal': 'legalModule',
          'reports': 'reportsModule',
          'loan': 'loanModule',
          'snagging': 'snaggingModule'
      };

      const flagKey = map[viewName];
      if (flagKey) {
          return featureFlags[flagKey] !== false; // Default to true if undefined, but respect explicit false
      }
  ...
  if (!currentUser) {
    return <div className="flex items-center justify-center h-screen text-slate-500 font-sans">Loading EstateFlow...</div>;
  }

  return (
    <div className="flex h-screen bg-[#F8F9FC] font-['Inter'] text-slate-900 overflow-hidden selection:bg-blue-100">
      ...
      <div className={`fixed inset-y-0 left-0 z-40 transform transition-transform duration-300 md:relative md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} shadow-[4px_0_24px_rgba(0,0,0,0.02)]`}>
        <Sidebar 
            currentView={view} 
        ...
        />
        {securityConfig.screenShieldEnabled && (
            <div className="absolute bottom-16 left-4 right-4 bg-red-900/80 p-3 rounded-lg border border-red-500/50 flex items-center gap-2 text-red-200 text-xs z-50">
      ...
      {/* Main Content Area */}
      <main className="flex-1 flex flex-col min-w-0 relative bg-[#F8F9FC]">
        
        {/* Top Navigation Header - Glassmorphism */}
        <header className="h-16 px-8 flex items-center justify-between backdrop-blur-md bg-white/70 border-b border-slate-200/60 sticky top-0 z-30 shadow-sm">
          ...
          <div className="flex items-center gap-4">
              ...
              <button 
                ...
                className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-slate-100 text-slate-500 rounded-lg text-sm hover:bg-white hover:shadow-sm border border-transparent hover:border-slate-200 transition group"
                title="Search (Cmd+K)"
              >
                  <Search className="w-4 h-4 group-hover:text-blue-600" />
                  <span className="text-xs">Search...</span>
                  <span className="text-[10px] font-mono border border-slate-300 px-1 rounded bg-white ml-2">⌘K</span>
              </button>
              <button onClick={() => setIsSearchOpen(true)} className="md:hidden p-2 text-slate-500"><Search className="w-5 h-5"/></button>
             ...
             <div className="relative">
               ...
               {showNotifications && (
                 <div className="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 ring-1 ring-black/5">
                   <div className="px-4 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50/80 backdrop-blur-sm">
                     ...
                     <button onClick={() => setNotifications([])} className="text-xs text-slate-400 hover:text-red-500 font-medium transition">Clear All</button>
                   </div>
                   <div className="max-h-80 overflow-y-auto custom-scrollbar">
                     {notifications.length === 0 ? (
                       ...
                       notifications.map(notif => (
                         <div 
                           key={notif.id} 
                           className={`p-4 border-b border-slate-50 hover:bg-blue-50/50 transition cursor-pointer ${notif.read ? 'opacity-60' : 'bg-blue-50/30'}`}
              ...
              <div className="relative">
                  <button 
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 bg-white/50 rounded-lg text-sm font-bold text-slate-700 hover:bg-white hover:shadow-sm border border-slate-200/60 transition backdrop-blur-sm"
                  >
                  ...
                  {showRoleSwitcher && (
                      <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 ring-1 ring-black/5">
                          <div className="px-4 py-2 bg-slate-50 border-b border-slate-100 text-xs font-bold text-slate-500 uppercase tracking-wide">Switch Perspective</div>
                          <button onClick={() => handleSwitchRole('SuperAdmin')} className="w-full text-left px-4 py-2 hover:bg-blue-50 text-sm hover:text-blue-600 transition">Super Admin</button>
                          <button onClick={() => handleSwitchRole('Presales')} className="w-full text-left px-4 py-2 hover:bg-blue-50 text-sm hover:text-blue-600 transition">Presales Agent</button>
                          <button onClick={() => handleSwitchRole('Sales')} className="w-full text-left px-4 py-2 hover:bg-blue-50 text-sm hover:text-blue-600 transition">Sales Manager</button>
                          <button onClick={() => handleSwitchRole('Reception')} className="w-full text-left px-4 py-2 hover:bg-blue-50 text-sm hover:text-blue-600 transition">Reception Desk</button>
                          <button onClick={() => handleSwitchRole('Banker')} className="w-full text-left px-4 py-2 hover:bg-blue-50 text-sm hover:text-blue-600 transition">Banker / Finance</button>
                          <div className="border-t border-slate-100 my-1"></div>
                          <button onClick={handleFactoryReset} className="w-full text-left px-4 py-2 hover:bg-red-50 text-sm hover:text-red-600 flex items-center gap-2 text-red-500 font-bold transition">
              ...
              <button 
                ...
                className="p-2 text-slate-500 hover:bg-white hover:shadow-sm hover:text-blue-600 rounded-full transition border border-transparent hover:border-slate-100"
                title="Help & Guide"
              >
              ...
              {view === 'leads' && (
                <button 
                  ...
                  className="text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 shadow-lg shadow-blue-500/20 transition hover:-translate-y-0.5 hover:shadow-blue-500/30 active:translate-y-0"
                  style={{ background: `linear-gradient(135deg, ${currentTenant.branding.primaryColor}, #1e40af)` }} // Gradient effect
                >
        ...
        {/* Module Viewport - Animated */}
        <div className="flex-1 overflow-auto p-6 relative scroll-smooth">
          <div className="max-w-[1600px] mx-auto w-full animate-in fade-in slide-in-from-bottom-3 duration-500 ease-out h-full">
            
            {/* Access Control Check */}
            {!isModuleAllowed(view) ? (
                <div className="flex flex-col items-center justify-center h-full text-slate-400 bg-white/50 rounded-2xl border border-slate-200/60 backdrop-blur-sm p-12">
                    <div className="bg-white p-8 rounded-full shadow-sm mb-4 ring-1 ring-slate-100">
                ...
                <>
                    ...
                    {view === 'bookings' && featureFlags.postSales && (
                        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm h-full overflow-hidden">
                            <BookingModule bookings={bookings} />
                        </div>
                    )}
                    {view === 'sales' && (
                    <div className="h-full overflow-hidden rounded-2xl border border-slate-200/60 shadow-sm">
                        <SalesModule 
                            projects={projects} 
                            leads={leads} 
                            inventory={inventory} 
                    ...
                    {view === 'visits' && featureFlags.visitEngine && (
                        <div className="h-full overflow-hidden rounded-2xl border border-slate-200/60 shadow-sm">
                            <SiteVisitModule 
                                siteVisits={siteVisits} 
                                projects={projects} 
                    ...
                    {view === 'loan' && featureFlags.loanModule && (
                        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm h-full overflow-hidden">
                            <LoanModule bookings={bookings} onUpdateBooking={handleUpdateBooking} currentUser={currentUser} />
                        </div>
                    )}
                    {view === 'snagging' && featureFlags.snaggingModule && (
                        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm h-full overflow-hidden">
                            <SnaggingModule bookings={bookings} projects={projects} />
                        </div>
                    )}

                    {view === 'reports' && featureFlags.reportsModule && (
                        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm h-full overflow-hidden">
                            <ReportsModule leads={leads} bookings={bookings} inventory={inventory} />
                        </div>
                    )}
                    ...
                    {view === 'developer-hub' && (
                        <DeveloperHub featureFlags={featureFlags} />
                    )}

                    {view === 'customer-portal' && (
                        <div className="bg-slate-100 rounded-2xl h-full overflow-hidden">
                            <CustomerPortal 
                                bookings={bookings}
                                projects={projects}
      ...
      <LeadModal 
        ...
        agents={agents}
        leadSources={leadSources}
        customFields={currentTenant.customFields || []} 
        leads={leads} 
      />
